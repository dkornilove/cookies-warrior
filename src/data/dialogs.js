...
export default {
  ...
  moveResult: ({ message: { monster, player } }) => {
    ...
    return ask(`Monster used ${redBright(name)} [${italic(desc)}].${player ? ` You used ${red(nameP)} and ${bold(descP)}` : ''}`);
  },
  passed: ([name, desc]) => ask(bold(`You have slashed the monster and got ${yellow(name)} [${italic(desc)}]`)),
